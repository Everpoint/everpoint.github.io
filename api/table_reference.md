...
- `referenceColumn` - системное имя _поля исходной таблицы_ (не атрибута слоя), по которому осуществляется атрибутивная связь (**обязательный параметр**).
...
- `tableReferences` - объект `tableReferences` для описания вложенных атрибутивных связей _присоединяемой таблицы_. Если они отсутствуют, остаётся пустым.
...
Создадим на основе этих таблиц 3 отдельных слоя (по одному на каждый показатель), в которых будут отображаться значения этих показателей в определённую дату и время (например, 12:00 08.12.2023). Для этого необходимо к каждому слою присоединить соответствующий атрибут с показателем (температура, влажность или давление), а также атрибут `datetime` для выбора измерений от 12:00 08.12.2023.
