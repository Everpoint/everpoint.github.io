define(["require", "exports", "./Control", "../FeatureLayer", "../features/Point", "../symbols/point/Point", "../geotools", "../Bbox"], function (require, exports, Control_1, FeatureLayer_1, Point_1, Point_2, geotools_1, Bbox_1) {
    "use strict";
     ...
     */
    class Snapping extends Control_1.Control {
        /**
        ...
        constructor(map, options = {}) {
             ...
             */
            this.snappingTypes = ['vertex', 'midpoint', 'line', 'axis', 'orthogonal'];
            /** Symbol of the snapping point */
            this.symbol = new Point_2.PointSymbol({ fillColor: 'red', size: 5 });
            /** Maximum distance in pixels from current point to the snapping point. */
            this.snappingDistance = 7;
        ...
        _onMouseMove(sGisEvent) {
            ...
            this._tempLayer.features = snapping ? [new Point_1.PointFeature(snapping.position, { crs: point.crs, symbol: this.symbol })] : [];
    ...
    let snapping = {
        vertex: function (point, layer, distance, activeFeature, activeRing, activeIndex) {
            let bbox = new Bbox_1.Bbox([point.x - distance, point.y - distance], [point.x + distance, point.y + distance], point.crs);
                ...
                if (feature.position) {
                    ...
                    for (let ring = 0; ring < rings.length; ring++) {
                        for (let j = 0; j < rings[ring].length; j++) {
                            if (features[i] === activeFeature && ring === activeRing && (Math.abs(j - activeIndex) < 2 || Math.abs(j - activeIndex) === rings[ring].length - 1))
                                continue;
            ...
            for (let i = 0; i < features.length; i++) {
                ...
                for (let ringIndex = 0; ringIndex < rings.length; ringIndex++) {
                    ...
                    for (let j = 1; j < ring.length; j++) {
                        if (features[i] === activeFeature && ringIndex === activeRing && (j === activeIndex || j - 1 === activeIndex || activeIndex === 0 && j === ring.length - 1))
                            continue;
            ...
            for (let i = 0; i < features.length; i++) {
                ...
                for (let ringIndex = 0; ringIndex < rings.length; ringIndex++) {
                    ...
                    for (let j = 1; j < ring.length; j++) {
                        if (features[i] === activeFeature && ringIndex === activeRing && (j === activeIndex || j - 1 === activeIndex || activeIndex === 0 && j === ring.length - 1))
                            continue;
                        let projection = geotools_1.pointToLineProjection(point.position, [ring[j - 1], ring[j]]);
        ...
        axis: function (point, layer, distance, activeFeature, activeRing = null, activeIndex = null) {
            if (!activeFeature || activeRing === null || activeIndex === null)
                return null;
            let lines = [];
            ...
            if (activeIndex === 0) {
                if (activeFeature.isEnclosed)
                    lines.push([ring[activeIndex], ring[ring.length - 2]]);
            }
            else {
                lines.push([ring[activeIndex], ring[activeIndex - 1]]);
            }
        ...
        orthogonal: function (point, layer, distance, activeFeature, activeRing = null, activeIndex = null) {
            ...
            for (let i = 0; i < lines.length; i++) {
                let projection = geotools_1.pointToLineProjection(point.position, lines[i]);
                ...
                if (Math.abs(dx) < distance && Math.abs(dy) < distance) {
                    ...
                    if (nextPoint && prevPoint) {
                        projection = geotools_1.pointToLineProjection(prevPoint, [ring[activeIndex], nextPoint]);
                        if (Math.abs(projection[0] - point.x) < distance && Math.abs(projection[1] - point.y) < distance) {
