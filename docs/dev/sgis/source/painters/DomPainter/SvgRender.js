define(["require", "exports", "../../renders/Arc", "../../renders/Poly", "../../utils/utils", "../../utils/Color"], function (require, exports, Arc_1, Poly_1, utils_1, Color_1) {
    "use strict";
    ...
    class SvgRender {
        ...
        _setNode() {
            if (this._baseRender instanceof Arc_1.Arc) {
            ...
            else if (this._baseRender instanceof Poly_1.PolyRender) {
                this._setPolyNode();
            }
        }
        _setPolyNode() {
            ...
            this._node = this._getPathNode({
                ...
                fill: this._baseRender.fillStyle === Poly_1.FillStyle.Color ? this._baseRender.fillColor : undefined,
                fillImage: this._baseRender.fillStyle === Poly_1.FillStyle.Image ? this._baseRender.fillImage : undefined,
                width: path.width,
                height: path.height,
                x: path.x,
                y: path.y,
    ...
    function setAttributes(element, attributes) {
        ...
        for (var i in attributes) {
            if (attributes.hasOwnProperty(i) && i !== 'fillImage' && attributes[i] !== undefined) {
                ...
                if (i === 'stroke' || i === 'fill') {
                    var color = new Color_1.Color(attributes[i]);
                    if (color.a < 255 || color.format === 'rgba') {
                        element.setAttribute(i, color.toString('rgb'));
                        if (color.a < 255)
                            element.setAttribute(i + '-opacity', color.a / 255);
                        continue;
                    }
                }
