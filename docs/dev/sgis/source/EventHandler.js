define(["require", "exports", "./utils/utils"], function (require, exports, utils_1) {
    ...
    var MouseEventFlags;
    (function (MouseEventFlags) {
        MouseEventFlags[MouseEventFlags["None"] = 0] = "None";
        MouseEventFlags[MouseEventFlags["MouseDown"] = 2] = "MouseDown";
        MouseEventFlags[MouseEventFlags["MouseUp"] = 4] = "MouseUp";
        MouseEventFlags[MouseEventFlags["MouseClick"] = 8] = "MouseClick";
        MouseEventFlags[MouseEventFlags["MouseMove"] = 16] = "MouseMove";
        MouseEventFlags[MouseEventFlags["MouseOver"] = 32] = "MouseOver";
        MouseEventFlags[MouseEventFlags["MouseOut"] = 64] = "MouseOut";
        MouseEventFlags[MouseEventFlags["DoubleClick"] = 128] = "DoubleClick";
        MouseEventFlags[MouseEventFlags["DragStart"] = 256] = "DragStart";
        MouseEventFlags[MouseEventFlags["Drag"] = 512] = "Drag";
        MouseEventFlags[MouseEventFlags["DragEnd"] = 1024] = "DragEnd";
    })(MouseEventFlags = exports.MouseEventFlags || (exports.MouseEventFlags = {}));
    exports.mouseEvents = {
    ...
    const eventTypeFlags = {};
    Object.keys(exports.mouseEvents).forEach(eventName => {
        eventTypeFlags[exports.mouseEvents[eventName].type] = exports.mouseEvents[eventName].flag;
    });
    /**
    ...
    class sGisEvent {
         ...
         */
        get isCanceled() { return this._cancelPropagation; }
    }
    exports.sGisEvent = sGisEvent;
    /**
    ...
    class EventHandler {
         ...
         */
        forwardEvent(event) {
            if (this._prohibitedEvents && this._prohibitedEvents.indexOf(event.type) !== -1)
                return;
            let eventType = event.type;
            if (this._eventHandlers && this._eventHandlers[eventType]) {
                let handlerList = utils_1.copyArray(this._eventHandlers[eventType]); //This is needed in case one of the handlers is deleted in the process of handling
        ...
        removeListener(description, handler) {
            ...
            let namespaces = getNamespaces(description);
            if (types.length === 0)
                types = Object.keys(this._eventHandlers);
            for (let i = 0; i < types.length; i++) {
                if (this._eventHandlers[types[i]]) {
                    for (let j = this._eventHandlers[types[i]].length - 1; j >= 0; j--) {
                        if ((namespaces === null || namespaces.length === 0 || utils_1.arrayIntersect(this._eventHandlers[types[i]][j].namespaces, namespaces)) &&
        ...
        allowEvent(type) {
            ...
            if (index !== -1)
                this._prohibitedEvents.splice(index, 1);
        }
        /**
        ...
        hasListener(type, handler) {
            if (this._eventHandlers[type]) {
                for (let i = 0; i < this._eventHandlers[type].length; i++) {
                    if (this._eventHandlers[type][i].handler === handler)
                        return true;
                }
            }
            return false;
                ...
                if (this._eventHandlers[types[i]] && this._eventHandlers[types[i]].length > 0) {
                    if (namespaces.length > 0) {
                        for (let j = 0; j < this._eventHandlers[types[i]].length; j++) {
                            if (utils_1.arrayIntersect(this._eventHandlers[types[i]][j].namespaces, namespaces)) {
                                return true;
                            }
                        }
                    }
         ...
         */
        on(description, handler, oneTime = false) { this.addListener.apply(this, arguments); }
        /**
         * @see EventHandler#removeListener
         */
        off(description, handler) { this.removeListener.apply(this, arguments); }
    }
